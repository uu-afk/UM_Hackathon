...
def recover_initial_request_without_glm(user_input: str) -> Dict[str, Any]:
    ...
    summary_parts = []
    if is_known_value(refund_details.get("customer_name")):
        summary_parts.append(f"Customer {refund_details['customer_name']}")
    else:
        summary_parts.append("Customer")

    if is_known_value(refund_details.get("order_id")):
        summary_parts.append(f"requests a refund for order {refund_details['order_id']}")
    else:
        summary_parts.append("requests a refund")

    if is_known_value(refund_details.get("product_or_service")):
        summary_parts.append(f"for {refund_details['product_or_service']}")

    if is_known_value(refund_details.get("refund_reason")):
        summary_parts.append(f"because of {refund_details['refund_reason'].replace('_', ' ')}")

    recovered_summary = " ".join(summary_parts).strip() + "."
    ...
    recovered_result = {
        ...
        "structured_output": {
            ...
            "eligibility_assessment": {
                "is_likely_eligible": False,
                "reason": "Refund details are ready for prototype verification.",
...
def recover_follow_up_without_glm(workflow: Dict[str, Any], answer: str) -> Dict[str, Any]:
    ...
    if not recovered_result.get("missing_information"):
        ...
        details = recovered_result["structured_output"].get("refund_details", {})
        recovered_result["structured_output"]["summary"] = (
            f"Customer refund request for order {details.get('order_id', 'unknown')} "
            f"regarding {details.get('product_or_service', 'the product')} is ready for validation."
        )
